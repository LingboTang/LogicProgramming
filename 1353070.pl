...
schedule(TimeLst,RmLst) :- write(' cannot be scheduled ').

% Constraint 1, find a different pair, and recursively permute each pair
...
% Constraint 2, find the pair, map time, I< J
...
% Constraint 3, find the [S,P,R] bounding
...
% Main: Map, domain, and get the result

subsetSum(L,R) :- map(L,R1,Map),domain(R1,0,1),
	resultingLst(L,R1,R).

% Constraint is sum ==0, label them, get the set, check if it empty.

resultingLst(L,R1,R) :-
	trickySum(L,R1,Sum),Sum #=0,labeling([ffc],R1), remove0(L,R1,G), R #= G,notEmpty(R).

% Add up everything by adding up the product of the actual number and flag.

trickySum([],[],0).
trickySum([A|CDRL],[B|CDRR],N):- 
	N#=N1+A*B,trickySum(CDRL,CDRR,N1).

remove0([],[],[]).
remove0([A|L],[B|S],K) :- B #= 1, remove0(L,S,K1), append([A],K1,K).
remove0([A|L],[B|S],K) :- B #= 0, remove0(L,S,K).
